...
class PretrainDataset(Dataset):
    ...
    def split_sequence(self):
        for seq in self.user_seq:
            ...
            self.part_sequence.append(input_ids)
...
class FDSADataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for item in input_ids:
            try:
                tags = self.args.item2tag[str(item)]

            except:
                tags = []

            tag_pad = self.args.max_tag_len - len(tags)
            tags = [0] * tag_pad + tags

            tags = tags[-self.args.max_tag_len:]
            assert len(tags) == self.args.max_tag_len

            input_tags.append(tags)

        assert len(input_ids) == self.max_len
        assert len(input_tags) == self.max_len
        assert len(target_pos) == self.max_len
        assert len(target_neg) == self.max_len
        ...
        if self.test_neg_items is not None:
            ...
            cur_tensors = (
                torch.tensor(user_id, dtype=torch.long), # user_id for testing
                torch.tensor(input_ids, dtype=torch.long),
                torch.tensor(input_tags, dtype=torch.long),
                torch.tensor(target_pos, dtype=torch.long),
                torch.tensor(target_neg, dtype=torch.long),
                torch.tensor(answer, dtype=torch.long),
                torch.tensor(test_samples, dtype=torch.long),
